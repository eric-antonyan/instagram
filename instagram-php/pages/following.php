...
<div class="users">
  <?php
  $getFollowings = q("SELECT * FROM `followers` WHERE follower_id = " . $user['id'] . "");
  while ($fuser = mysqli_fetch_assoc($getFollowings)) { ?>
    <a href="../<?= getUser($fuser['followed_id'])['username'] ?>">
      <div class="user">
        <div class="left">
          <img src="../users/<?= getUser($fuser['followed_id'])['pic'] ?>" alt="" class="chat__user-pic">
        </div>
        <div class="right">
          <span class="top"><?= getUser($fuser['followed_id'])['full_name'] ?></span>
          <span class="bottom"><?= getUser($fuser['followed_id'])['username'] ?></span>
        </div>
      </div>
    </a>
  <?php }
